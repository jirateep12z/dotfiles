...
GetYoutubeDownloadDirectory() {
  ...
  if [[ -z "$youtube_download_dir" ]]; then
    LogError "Unable to determine youtube download directory for OS: $SYSTEM_OS_TYPE"
    LogInfo "Please run from a supported shell or configure the download directory"
    exit 1
  fi
...
OpenYoutubeDownloadDirectory() {
  ...
  local open_command=()
  local open_path="$youtube_download_dir"
  if [[ $OSTYPE == "darwin"* ]]; then
    open_command=("open")
  elif [[ $OSTYPE == msys* ]] || [[ $OSTYPE == mingw* ]] || [[ $OSTYPE == cygwin* ]] || [[ $SYSTEM_OS_TYPE == MINGW* ]] || [[ $SYSTEM_OS_TYPE == MSYS* ]] || [[ $SYSTEM_OS_TYPE == CYGWIN* ]]; then
    open_path="$(cygpath -w "$youtube_download_dir" 2>/dev/null || echo "$youtube_download_dir")"
...
InteractiveMenu() {
  ...
  if [[ "$DOWNLOAD_PLAYLIST" == "true" ]]; then
    ...
    read -r -p "Download entire playlist? (Y/N, default: Y): " playlist_choice
    if [[ "${playlist_choice^^}" == "N" ]]; then
  ...
  read -r -p "Download video or audio? (V/A, default: V): " DOWNLOAD_TYPE
  DOWNLOAD_TYPE="${DOWNLOAD_TYPE^^}"
  if [[ -z "$DOWNLOAD_TYPE" ]]; then
    DOWNLOAD_TYPE="V"
  elif [[ ! "$DOWNLOAD_TYPE" =~ ^[VA]$ ]]; then
  ...
  if [[ "$DOWNLOAD_TYPE" == "V" ]]; then
    ...
    read -r -p "Download subtitles? (Y/N, default: N): " subtitle_choice
  ...
  read -r -p "Proceed with download? (Y/N, default: Y): " confirm_download
  if [[ "${confirm_download^^}" == "N" ]]; then
