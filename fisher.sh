...
source "$SCRIPT_DIR/utils/fish/logger.fish" 2>/dev/null
if test $status -ne 0
  echo "Error: Cannot load logger.fish utility" >&2
  exit 1
end
...
function InstallFisherPlugins
  ...
  for plugin in $plugins
    ...
    if test $status -ne 0
      LogError "Failed to install Fisher plugin: $plugin"
      set -a failed_plugins $plugin
    end
  end
  if test (count $failed_plugins) -gt 0
    set failed_summary (string join ', ' $failed_plugins)
    LogError "Failed to install Fisher plugins: $failed_summary"
    return 1
  end
...
function UninstallFisherPlugins
  ...
  for plugin in $plugins
    ...
    if test $status -ne 0
      LogError "Failed to uninstall Fisher plugin: $plugin"
      set -a failed_plugins $plugin
    end
  end
  if test (count $failed_plugins) -gt 0
    set failed_summary (string join ', ' $failed_plugins)
    LogError "Failed to uninstall Fisher plugins: $failed_summary"
    return 1
  end
...
function InstallNPMPackages
  ...
  for package in $packages
    ...
    if test $status -ne 0
      LogError "Failed to install NPM package: $package"
      set -a failed_packages $package
    end
  end
  if test (count $failed_packages) -gt 0
    set failed_summary (string join ', ' $failed_packages)
    LogError "Failed to install NPM packages: $failed_summary"
    return 1
  end
...
function UninstallNPMPackages
  ...
  for package in $packages
    ...
    if test $status -ne 0
      LogError "Failed to uninstall NPM package: $package"
      set -a failed_packages $package
    end
  end
  if test (count $failed_packages) -gt 0
    set failed_summary (string join ', ' $failed_packages)
    LogError "Failed to uninstall NPM packages: $failed_summary"
    return 1
  end
...
function InstallAll
  set failed_steps
  if not InstallFisher
    set -a failed_steps "Fisher"
  end
  if not InstallFisherPlugins
    set -a failed_steps "Fisher plugins"
  end
  if not InstallNodeJS
    set -a failed_steps "Node.js"
  end
  if not InstallNPMPackages
    set -a failed_steps "NPM packages"
  end
  if test (count $failed_steps) -gt 0
    set failed_summary (string join ', ' $failed_steps)
    LogError "Package installation failed: $failed_summary"
    return 1
  end
...
function UninstallAll
  set failed_steps
  if not UninstallNPMPackages
    set -a failed_steps "NPM packages"
  end
  if not UninstallNodeJS
    set -a failed_steps "Node.js"
  end
  if not UninstallFisherPlugins
    set -a failed_steps "Fisher plugins"
  end
  if test (count $failed_steps) -gt 0
    set failed_summary (string join ', ' $failed_steps)
    LogError "Package uninstallation failed: $failed_summary"
    return 1
  end
